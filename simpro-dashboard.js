...
function dbSwitchTab(sectionName){
  ...
  // Daftar PO juga dimuat SAAT TAB DIBUKA -- alasannya sama: daftarnya bisa
  // ratusan baris dan tidak semua sesi dashboard membutuhkannya.
  if(sectionName === "daftarpo" && typeof dbMuatDaftarPO === "function") dbMuatDaftarPO();
}
...
function dbBukaEditPO(idPO){
  ...
  window.DB_EDITPO_ID = idPO;

  fetch(DB_API_URL, {
    method: "POST",
    body: JSON.stringify({ idToken: DB_ID_TOKEN, action: "getPOUntukEdit", idPurchaseOrder: idPO })
  })
...
function dbMuatProgresWarna_(idPO){
  ...
  fetch(DB_API_URL, {
    method: "POST",
    body: JSON.stringify({ idToken: DB_ID_TOKEN, action: "getProgresWarnaPO", idPurchaseOrder: idPO })
  })
  .then(function(r){ return r.json(); })
  .then(function(d){
    if(!d || !d.success){
...
function dbTambahWarnaPO(btn, idxItem){
  ...
  const it = (window.DB_EDITPO_DATA.itemGroups || [])[idxItem];
  if(!it || !tbody) return;
...
function dbSimpanEditPO(){
  ...
  fetch(DB_API_URL, {
    ...
    body: JSON.stringify({
      ...
      action: "simpanEditPO",
      payload: {
        idPurchaseOrder: window.DB_EDITPO_ID,
...
function dbMuatDaftarPO(){
  if(window.DB_DAFTAR_PO) { dbRenderDaftarPO(); return; }
  fetch(DB_API_URL, {
    method: "POST",
    body: JSON.stringify({ idToken: DB_ID_TOKEN, action: "getDaftarPO" })
  })
  .then(function(r){ return r.json(); })
  .then(function(d){
    if(!d || !d.success){
    ...
    const statusUnik = [];
    window.DB_DAFTAR_PO.forEach(function(p){
...
function dbRenderDaftarPO(){
  ...
  if(!wadah) return;
  const semua = window.DB_DAFTAR_PO || [];
